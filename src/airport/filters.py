...
class FlightFilter(django_filters.FilterSet, PaginationFilter):
    departure_time = django_filters.DateTimeFilter(
...
class TicketFilter(django_filters.FilterSet, PaginationFilter):
    flight = django_filters.CharFilter(
        field_name="flight__route__source__name", lookup_expr="icontains"
    )
    order_user = django_filters.CharFilter(
        field_name="order__user__username", lookup_expr="icontains"
    )
    page_size = django_filters.NumberFilter(
        field_name="page_size",
        method="filter_page_size",
